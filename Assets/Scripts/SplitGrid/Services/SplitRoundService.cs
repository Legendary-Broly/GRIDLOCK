...
namespace SplitGrid.Services
{
    public class SplitRoundService : ISplitRoundService
    {
        ...
        {
            return progressTracker.IsGridComplete(GridID.GridA) && 
                   progressTracker.IsGridComplete(GridID.GridB);
        }

        public void SetGridDimensions(GridID gridId, int width, int height)
        {
            splitGridService.InitializeGrids(width, height);
        }
        ...
        {
            progressTracker.NotifyFragmentRevealed(gridId, x, y);
        }
